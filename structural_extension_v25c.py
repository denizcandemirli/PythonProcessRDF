...
# S1–S4 Structural Extension (v25b -> v25c)
# - Expanded IFC/type regex (adds IfcWallStandardCase, keeps brace/beam/column/slab synonyms)
# - Optional debug artifacts: functions pivot, motif densities, score components (--emit-debug)
# - Preserves: M5 hotfix injection from S1, --func-all for S1 gating bypass, dual calibration
#
...
#   struct_system_scores.csv           (incl. dual_raw, dual, frame_share)
...
RE_IFC_WALL   = re.compile(r'(IfcWall(StandardCase)?|(^|[^A-Za-z])(Wall|Shear[- ]?Wall|CoreWall|RetainingWall)([^A-Za-z]|$))', re.I)
RE_IFC_BRACE  = re.compile(r'(IfcStructuralCurveMember|(^|[^A-Za-z])(Brace|Bracing|X[- ]?brace|K[- ]?brace|Chevron|Tie|Strut)([^A-Za-z]|$))', re.I)
...
def s1_inventories(models: List[ModelData], out_dir: str, func_all: bool, emit_debug: bool):
    ...
    for m in models:
        ...
        for e, flist in m.functions.items():
            if (not func_all) and (not any(is_struct_element(t) for t in m.types.get(e, []))):
                continue
            for f in flist:
                fb = func_bucket(f)
...
# ---------- S2 (motifs) ----------
def s2_motifs(models: List[ModelData], out_dir: str, emit_debug: bool):
    ...
    for m in models:
        ...
        # structRole motif: E_* -> F_(load|shear|moment) — structural gating kept
        role_edges = 0
        for e, flist in m.functions.items():
...
def inject_M5_from_s1(df_counts: pd.DataFrame, df_funcs: pd.DataFrame) -> pd.DataFrame:
    """
    S1 fonksiyon histogramından (LoadBearing/Shear/Moment) M5_structRole sayısını türet,
    motif tablosuna yaz. Böylece S2 tarafında gating/reindex gibi nedenlerle kaybolan M5 sinyali korunur.
    """
...
def s3_system_scores(models: List[ModelData],
    ...
    for m in dens.index:
        ...
        rows_comp.append({
            "model": m,
            "frame_motif": frame_motif, "frame_func": frame_func,
            "wall_m3": wall_m3, "wall_func": wall_func
        })
...
def main():
    ...
    ap.add_argument("--out-name", default="07 - Structural_Extension_v25c")
    ...
    ap.add_argument("--func-all", action="store_true",
                    help="If set, count functions on ALL elements in S1 (not only structural).")
    ap.add_argument("--emit-debug", action="store_true",
                    help="If set, writes additional diagnostics: functions pivot, motif densities, score components.")
    args = ap.parse_args()
    ...
    print(f"[LOAD] (Structural Extension {VERSION})")
    ...
    # S2
    motif_counts, motif_shares = s2_motifs(models, out_dir, emit_debug=args.emit_debug)

    # ---- M5 HOTFIX CALL (from S1) ----
