# structural_extension_v25.py  (v25b)
# S1–S4 Structural Extension:
# - Expanded proxies for Wall–Slab: strict intersect, adjacency, zone-based adjacency
...
# - NEW: M5 hotfix injection right after S2
# - NEW: --func-all flag to count functions on all elements (not only structural)
#
...
#   struct_system_scores.csv   (includes dual_raw, dual, frame_share)
...
# ---------- S1 ----------
def s1_inventories(models: List[ModelData], out_dir: str, func_all: bool):
...
def s2_motifs(models: List[ModelData], out_dir: str):
    ...
    for m in models:
        ...
        role_edges = 0
        for e, flist in m.functions.items():
            # NOTE: in motif land we keep structural gating (semantik tutarlılık)
...
def s3_system_scores(models: List[ModelData],
    ...
    # DEBUG: show LB shares before scoring
    debug_lb = func_hist[func_hist["function"]=="LoadBearing"][["model","share"]].to_dict("records") if not func_hist.empty else []
    print("[DEBUG] LoadBearing shares:", debug_lb)
...
def main():
    ...
    # S1
    df_types, df_funcs = s1_inventories(models, out_dir, func_all=args.func_all)
    # S2
