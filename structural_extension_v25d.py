...
# Structural Extension S1–S4 (v25c -> v25d)
# - Wider IFC/regex type capture (Beam/Column/Slab/Wall/Brace/Core/Foundation + Ifc*)
# - Wider predicate localnames (adjacentElements, intersectsElement, basic zone aliases)
# - M5 injection ordering guaranteed (write AFTER injection)
# - S4 similarity writing fixed (labels aligned, weights applied)
# - Data availability diagnostic: struct_data_availability.csv
#
...
def s1_inventories(models: List[ModelData], out_dir: str, func_all: bool, emit_debug: bool):
    ...
    for m in models:
        # structural element nodes by regex/IFC
        ...
        # function histogram (gating optional)
...
def inject_M5_from_s1(df_counts: pd.DataFrame, df_funcs: pd.DataFrame) -> pd.DataFrame:
    """Inject M5 from S1 function histogram (LB/Shear/Moment counts)."""
...
def main():
    ...
    ap.add_argument("--out-name", default="07 - Structural_Extension_v25d")
    ...
    ap.add_argument("--emit-debug", action="store_true",
                    help="If set, writes additional diagnostics.")
    args = ap.parse_args()

    out_dir = os.path.join(args.out_root, args.out_name); ensure_dir(out_dir)
    # tolerate accidental arg-name variant
